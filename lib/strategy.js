...
Strategy.prototype.userProfile = function(accessToken, done) {
	...
	this._oauth2.get(this._profileURL, accessToken, function (err, body, res) {
		...
		profile.id = parsed.key.replace(/[^0-9]/gi, "");
		profile.name = parsed.name;
		profile.displayName = parsed.name;
		profile.email = parsed.account;
		profile.image = parsed.profile.image;
		profile.text = parsed.profile.text;
		profile.exordial = parsed.profile.text;
